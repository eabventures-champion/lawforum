...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Configure a new navigation link or parent dropdown menu for the site header.</p>
    </div>
...
<div class="card-table" style="max-width: 900px; padding: 32px; border-radius: 20px;">
    <form action="{{ route('navigation-menus.store') }}" method="POST">
        @csrf

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
            <div class="form-group">
                <label for="title" class="form-label">Menu Title (Required)</label>
                <input type="text" id="title" name="title" class="form-control" placeholder="e.g. Constitutional Court" value="{{ old('title') }}" required>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-top: 16px;">
            <div class="form-group">
                ...
                <select id="link_type" name="link_type" class="form-control">
                    <option value="url" {{ old('link_type', 'url') == 'url' ? 'selected' : '' }}>Direct Route / URL Link</option>
                    <option value="content" {{ old('link_type') == 'content' ? 'selected' : '' }}>Custom Dynamic Page Content</option>
                </select>
        ...
        <div class="form-group" id="url_group" style="margin-top: 16px;">
            ...
            <input type="text" id="url" name="url" class="form-control" placeholder="e.g. /constitution/Republic/Ghana/1, /judgement/Ghana" value="{{ old('url') }}">
        ...
        <div class="form-group" id="content_group" style="margin-top: 16px; display: none;">
            ...
            <textarea id="custom_content" name="custom_content" class="form-control" style="min-height: 300px; font-family: monospace;" placeholder="<h1>Write page title or content here...</h1><p>This is a custom page body</p>">{{ old('custom_content') }}</textarea>
        ...
        <div class="setting-switch-wrapper" id="dropdown_switch_group" style="margin-top: 24px;">
            <label class="switch">
                <input type="checkbox" id="is_dropdown" name="is_dropdown" value="1" {{ old('is_dropdown') == '1' ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // (A menu item cannot be a dropdown parent if it's already a child of another menu)
