...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Configure a new pricing tier, duration, download quota, card badge, features, and button text.</p>
    </div>
    ...
    <form action="{{ route('admin.subscriptions.store') }}" method="POST" id="subscriptionForm">
        ...
        <div style="margin-bottom: 28px;">
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="text" name="type" value="{{ old('type') }}" required placeholder="e.g. Starter, Professional, Business" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    @error('type')
                ...
                <div>
                    ...
                    <input type="text" name="currency" value="{{ old('currency', 'GHS') }}" placeholder="GHS" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    @error('currency')
                        <span style="color: #ef4444; font-size: 13px; margin-top: 4px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="price" value="{{ old('price') }}" required placeholder="e.g. 60.00" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    @error('price')
                        <span style="color: #ef4444; font-size: 13px; margin-top: 4px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div style="margin-bottom: 28px; padding-top: 20px; border-top: 1px solid var(--border-color);">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="number" min="1" name="duration" value="{{ old('duration', 90) }}" required placeholder="e.g. 90 (3 months) or 365 (1 year)" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                ...
                <div>
                    ...
                    <input type="text" name="duration_text" value="{{ old('duration_text') }}" placeholder="e.g. for 3 months, for 6 months, per year" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    <small style="display: block; color: var(--text-secondary); margin-top: 4px;">Subtext displayed right under the price amount on the card (leave empty to auto-calculate).</small>
                    @error('duration_text')
                        <span style="color: #ef4444; font-size: 13px; margin-top: 4px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="number" min="0" name="no_downloads" value="{{ old('no_downloads', 50) }}" required placeholder="e.g. 50 or 20000 for unlimited" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    <small style="display: block; color: var(--text-secondary); margin-top: 4px;">User's actual download allowance in the system.</small>
                    @error('no_downloads')
                        <span style="color: #ef4444; font-size: 13px; margin-top: 4px; display: block;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="highlight_text" value="{{ old('highlight_text') }}" placeholder="e.g. Up to 50 document downloads" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                    <small style="display: block; color: var(--text-secondary); margin-top: 4px;">Text inside the blue badge pill above the bullet points list (leave empty to auto-calculate).</small>
        ...
        <div style="margin-bottom: 28px; padding-top: 20px; border-top: 1px solid var(--border-color);">
            ...
            <div id="featuresListContainer" style="display: flex; flex-direction: column; gap: 10px;">
                @php
                    $defaultFeatures = old('features', [
                        'Free Access and Downloads to Constitution and Pre-4th Republic Laws',
                        'Download 50 documents (4th Republic Laws and Cases) for 3 months',
        ...
        <div style="margin-bottom: 28px; padding-top: 20px; border-top: 1px solid var(--border-color);">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                <div>
                    ...
                    <input type="text" name="badge" value="{{ old('badge') }}" placeholder="e.g. Most Popular, Best Value, Recommended" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
                ...
                <div>
                    ...
                    <input type="text" name="button_text" value="{{ old('button_text', 'Subscribe Now') }}" placeholder="e.g. Subscribe Now, Get Started" style="width: 100%; padding: 11px 14px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 14px; outline: none;">
            ...
            <div style="display: flex; gap: 32px; align-items: center; flex-wrap: wrap; padding: 16px 20px; background: rgba(255, 255, 255, 0.02); border: 1px solid var(--border-color); border-radius: 12px;">
                ...
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 14px; font-weight: 600; color: var(--text-primary);">
                    <input type="checkbox" name="is_popular" value="1" {{ old('is_popular', false) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: #3b82f6; cursor: pointer;">
        ...
        <div style="display: flex; gap: 12px; margin-top: 32px;">
            <button type="submit" class="btn btn-primary btn-action" style="padding: 12px 28px; font-size: 15px;">
                <i class="fa-solid fa-check"></i> Create Subscription Plan
            </button>
