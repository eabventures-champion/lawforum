...
<body>
    ...
    <div class="header_only" style="border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.03); border-radius: 8px; display: flex; justify-content: space-between; align-items: center; padding: 10px 16px; margin-bottom: 12px;">
        <span style="font-weight: 700; font-size: 15px; color: #60a5fa;">{{ $amendedAct['section'] }}</span>
        @php
            $isBookmarked = false;
            if (auth()->check()) {
                $isBookmarked = \App\UserBookmark::where('user_id', auth()->id())
                    ->where(function($q) use ($amendedAct) {
                        $q->where('section_id', $amendedAct['id'])
                          ->orWhere('user_section', auth()->id() . '_legislation_' . $amendedAct['act_id'] . '_' . $amendedAct['id']);
                    })->exists();
            }
        @endphp
        <button type="button" 
                class="btn-bookmark-toggle {{ $isBookmarked ? 'is-bookmarked' : '' }}" 
                data-act-title="{{ $amendedAct['post_act'] }}" 
                data-act-section="{{ $amendedAct['section'] }}" 
                data-section-id="{{ $amendedAct['id'] }}" 
                data-act-id="{{ $amendedAct['act_id'] }}" 
                data-act-group="{{ $amendedAct['act_group'] ?? 'Acts of Parliament' }}" 
                data-doc-type="legislation" 
                data-page-url="/new-laws/amended-acts-table-of-content/{{ $amendedAct['act_group'] }}/{{ $amendedAct['post_act'] }}/{{ $amendedAct['act_id'] }}"
    ...
    <div style="margin-bottom: 12px;">
        <div class="menu_options" style="display: flex; gap: 12px; justify-content: flex-end; align-items: center;">
            @if (Route::has('login'))
                @auth
                    <a class="download_link" href="javascript:;" rel="/new-laws/amended_acts/pdf-section-content/{{$amendedAct['post_act']}}/{{ $amendedAct['id'] }}" style="color: #60a5fa; text-decoration: none; font-size: 13px;"><img alt="PDF" src="{{ asset('/logo/pdf.png') }}" style="width:1.3em; vertical-align: middle;">&nbsp;PDF</a>
                    <a href="/new-laws/amended-acts/print_section_content/{{ $amendedAct['id'] }}" target="_blank" style="color: #60a5fa; text-decoration: none; font-size: 13px;"><i class="fa-solid fa-print"></i>&nbsp;Print</a>
                @else
