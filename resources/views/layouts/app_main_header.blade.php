...
<html lang="en" style="background-color:#070a13;">
  <head>
    ...
    <meta name="description" content="Browse and search Ghana case laws including Supreme Court, Court of Appeal, and High Court judgements on Legals Forum.">
    <title>Ghana Case Laws - Legals Forum</title>
  ...
  <body>
    ...
    <nav class="nav-wrap" id="mainNav">
        <div class="nav-inner">
            ...
            <button class="mobile-menu-btn" onclick="document.querySelector('.nav-menu-links-premium').classList.toggle('show')">
                        ...
                        <table class="table table-hover" id="datatable" style="width: 100%;">
                            ...
                            <tbody>
                              @foreach($ghlawjudgments as $ghlawjudgment)
                                <tr>
                                    <td>
                                        <a href="/judgement/Ghana/{{ $ghlawjudgment->gh_law_judgment_group_name }}/{{ $ghlawjudgment->id}}">{{ $ghlawjudgment->case_title }}</a>
                                    </td>
    ...
    <script>
        $(document).ready(function(){
            ...
            // Intercept tab clicks for AJAX content switching (no page refresh)
            $('.nav-underline-premium').on('click', 'a.nav-link-premium', function(e) {
                ...
                // Determine court type from href
                var court = 'all';
                if (href.indexOf('/1/Supreme-Court') !== -1) court = 'Supreme-Court';
                else if (href.indexOf('/3/Court-of-Appeal') !== -1) court = 'Court-of-Appeal';
                else if (href.indexOf('/2/High-Court') !== -1) court = 'High-Court';
                ...
                var headerMap = {
                    'all': {
                        h1: 'Ghana Case Laws',
                        p: 'Browse and search Ghana case laws including Supreme Court, Court of Appeal, and High Court judgements.'
                    },
                    'Supreme-Court': {
                        h1: 'Supreme Court',
                        p: 'Browse and search Ghana Supreme Court case laws and judgements.'
                    },
                    'Court-of-Appeal': {
                        h1: 'Court of Appeal',
                        p: 'Browse and search Ghana Court of Appeal case laws and judgements.'
                    },
                    'High-Court': {
                        h1: 'High Court',
                        p: 'Browse and search Ghana High Court case laws and judgements.'
                    }
                };

                if (headerMap[court]) {
                    $('h1.page-title').text(headerMap[court].h1);
                    $('p.page-subtitle').text(headerMap[court].p);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data && response.data.length > 0) {
                            response.data.forEach(function(item) {
                                table.row.add([
                                    '<a href="' + item.url + '">' + item.case_title + '</a>',
                        ...
                        history.pushState({ court: court }, '', href);
                    ...
                    error: function() {
                        $('#tableLoader').removeClass('show');
                        alert('Failed to load case law data. Please try again.');
                    }
                });
            });
            ...
            document.addEventListener('click', (e) => {
                const menu = document.querySelector('.nav-menu-links-premium');
